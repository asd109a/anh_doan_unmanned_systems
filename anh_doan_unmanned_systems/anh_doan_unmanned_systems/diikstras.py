...
class Obstacle():
    ...
    def is_inside(self, curr_x: float, curr_y: float, robot_diameter: float) -> bool:
        # Inflate the obstacle by half of the robot's diameter
...
while [current_node.x, current_node.y] != [goal_x, goal_y]:
    current_idx = min(unvisited, key=lambda x: unvisited[x].cost)  # Set current_index to min value from unvisited
    current_node = unvisited[current_idx]  # Set current_node to unvisited[current_index]
    visited[current_idx] = current_node  # Put current_node into visited dictionary
    del unvisited[current_idx]  # Delete current_index from unvisited
...
plt.title("Dijkstras Algorithm Path Finding")
...
print(f"Time taken to execute Dijkstras algorithm: {execution_time} seconds")
